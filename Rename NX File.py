...
def nx_file_renaming(path_to_nx_file_to_rename, new_name):

    # Take the Name that would be used to rename the file
    new_name = new_name + ".prt"
    ...
    path_to_nx_file_to_rename = path_to_nx_file_to_rename.replace("/", "\\")

    # Collect All other NX files in the same directory
    nx_files = []

    shell = Dispatch("Shell.Application")
    nx_comobject = shell.NameSpace(directory)
    for item in nx_comobject.Items():
        # Check for Assembly file and add it to the list
        if item.Path.endswith(".prt") and nx_comobject.GetDetailsOf(item,322) == "Assembly":
            nx_files.append(item.Path)
    if path_to_nx_file_to_rename in nx_files:
        nx_files.remove(path_to_nx_file_to_rename)
        ...
        if output:
            ...
            for child in childs:
                if nx_file_to_rename in child:
                    rename_arg = arg_first_section + arg_second_section + " -o " + arg_second_section + " -change_name " + f'"{nx_file_to_rename}" ' + f'"{new_name}"'
    ...
    to_name = os.path.join(directory, new_name)
...
class App:
    ...
    def show_about(self):
        ...
        about_label = tk.Label(about_window, text=info, justify="left", wraplength=550)
        about_label['background']='#00e2a2'
        about_label.grid(row=0, rowspan=7, column=0, columnspan=4, padx=15, pady=10)
